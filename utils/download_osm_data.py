    ...
    for address in addresses:
        if check_to_add(address):
            address = dict(
                ...
                street=address.tags['addr:street'].replace('улица', '').strip(),
...
def find_region_of_city(name):
    ...
    first_search = re.search(r'<table class="standard sortable"><tbody><tr>(.+?)</td></tr></tbody></table>', req.text,
                             re.DOTALL)
    try:
...
def check_to_add(address):
    ...
    return 'addr:housenumber' in keys and 'addr:street' in keys and 'lat' in keys and 'lon' in keys
